...
> 把请求压入队列，交给事件分派器，分派其交给对应的处理器
 ...
 ==光答内存和数据结构优化和快速访问不够==
> [!NOTE]
> redis快的原因面试可以答: 空间换时间(SDS数据类型,Zset SkipList数据结构, 过期key的清理策略), 单次改批量(pipeLine), 各取所长( ZSet 数据类型) , 同步非阻塞(Reactor模型) , 同步转异步(解放主线程)  
...
> [!NOTE]
> 面试这么答: 
> redis 设置的目的就是为了抗更高的并发和更快的数据服务
>先更新redis 不能保证按顺序更新,假如有一个频繁更新的数据就会出错,当然出错的概率还是很低的 ,   而且数据容易过期
> 先更新MySQL 再更新Redis 的方法 请求会拿到过期的数据,有一致性问题
> 先更新Mysql 再删除Redis  读到旧数据 
> 先删除再更新 Mysql  这样做可以让两份数据变成一份,也就没有了一致性问题.  删除Redis 成功但是 更新MySQL失败, 返回给用户的也得是失败; 删除redis失败了, 返回用户失败,因为你在保持一致性的工作中失败了,就得告诉用户你失败了, 面对失败的成本是最低的
>而且假如有线程在删完redis但是没更新mysql的时候拿到了mysql的数据,并把老数据加载到redis里, 这个时候就要再次删除redis, 也就是延时双删 . 如果这个时候双删失败了, 最好的方法依然是面对失败

==一致性要求的是最终数据要保持一致, 哪怕读到旧数据没有关系可以容忍. 比如会计做帐做的慢,但是最后都要对账做对 . (最终数据满足一致约束) 你可以慢但是不能错==

> 先redis 都会有redis 快而导致时间差. 在这个时间内, 数据库没有完成更新,redis会过期导致mysql更新失败
> 或者会读到数据库旧数据又放入到redis里 ,延时双删也会出现错误,那就真一致性问题了

### 高一致性场景如何做, 一定会选择先删除Redis,这样做又会又删除删不掉的问题
 支付系统, 库存管理系统 常见
...
3. 简单业务可以使用延迟删
    ...
    `` 
  延迟双删要顾及合理的TTL , 保证就算最后的删除失败也要让TTL 尽可能减小影响 
...
> [!NOTE]
> 更先进的回答: 本地缓存的4种更新方案:
> 1. 所有服务器API调用:   微服务架构是通过Ribbon/Spring CLoud LoadBalance 负载均衡,更新本地缓存的请求只会打到一台服务器上,此时要通过服务发现接口 获得集群中的所有应用服务器地址,并全部进行调用更新 ,==没有外部依赖,数据实时性高. 仅支持程序的方式主动更新==
> 2. 配置中心监听:   Nacos,Eureca 等配置中心的特性完成本地缓存的更新. 任意一台服务器接收到本地缓存更新请求,配置中心API完成配置变更,所有服务器进行配置变化来完成本地缓存的更新  ==有数据时延性,非常依赖配置中心,不支持大量数据的本地缓存, 方式可以是程序主动更新,也可以登录配置中心完成配置变更==
> 3. 消息队列广播:    任何一台服务器接收到更新请求,往消息队列方一个广播消息,服务器消费该消息完成本地缓存的更新 ==数据时延更高,只支持程序主动更新==
> 4. XXL-JOB分片广播:  不再像前三种通过请求更新,而是通过定时任务完成. 通过xxl-job调度中心往服务器发送请求 来触发本地缓存更新 ,调度中心挂了也可以手动发HTTP请求触发各种执行器(服务器)来更新
 ==适合对数据时延性要求不高的场景==
...
### 热点key的防治
1. 把热点key放到redis外面减小redis的压力
2. 拆分key , 分成多个子key, 随机放到不同的节点上面

### 处理大Key
大key 占用内存空间,主从同步时会延迟 , 也有可能耗光内存

把它拆分成多个key , 每个条目一个key
Json数据可以先字符流后压缩(Gzip), 压缩完后存储

> [!NOTE]
> Redis4.0 还引入了del的异步版本: unlink, 
> 之后的del操作不再是阻塞式的
> ==这种同步转异步的思想也是提升性能的方式==


## 缓存预热
 系统启动前 把热点key提前放到redis里

 == 在技术派实战项目中，我会在项目启动时将热门文章提前加载到 Redis 中，在每天凌晨定时将最新的站点地图更新到 Redis中，以确保用户在第一次访问时就能获取到缓存数据，从而减轻数据库的压力。
## 无底洞问题 

 缓存节点数量增加,总容量变大,吞吐量提高 . 但是单个请求的响应时间变长
 ==本质上时数据碎片化,请求要访问的节点变多== , 网络通信的开销提高

1. 同一个节点的多个请求合并成一个批量请求,减少网络往返次数(所有访问的keys放到List里面,用stream API 和 CompletabelFuture 流式处理异步访问,把所有的结果集合封装到Map<String,Object>里面, 每个Object也是一个Map<String,Object>)
2. 使用一致性哈希算法


# Redis 运维
 ## 内存不足怎么办
 查看内存不足: `redis-cli Info Memory`
 提高内存上限: maxmemory 8gb
 修改maxmemory-policy 内存淘汰策略  `maxmemory-policy allkeys-lru`
...
## key过期策略
 ...
 redis把每个==设置了过期时间的key==放到==一个独立的字典==中,以后会定期遍历这个字典删除过期key, 默认每秒十次扫描,扫描过程不会遍历字典,而是采用贪心策略,随机抽20key, 删除过期key,如果过期key的占比达到1/4, 就重复这个过程

该线程不仅处理定时任务,还负责处理客户端请求,而且为了保证过期扫描不会出现过度循环(占比超过1/4 重复过程),导致客户端请求堆积,和处理缓慢,还增加了扫描时间的上限,不会超过25ms

##### 定期清理设置25ms上限和 惰性删除机制 也是一种空间换时间的策略,让客户端的响应时间得到保证
  毕竟人家让你不发现不删和 删除不能妨碍客户端链接,过期key得不到删除 自然浪费了空间


## redis 内存淘汰策略
 8种: noeviction(不驱逐),内存不足时报错不删除
   设置过期时间的: volatile-random 随机删除过期key
        volatile-ttl  删除剩余时间最短的key
        volatile-lru 删除最近最少使用的key
        volatile-lfu 删除访问次数最少的key     
   淘汰所有数据: 
        allkeys-lru 保留最近使用的key,删除最近最少使用的key
        allkeys-lfu 保留常用key, 删除最不常用的key
        allkeys-random 随机删除key

## redis发生阻塞?
 monitor 命令查看正在执行的命令, slowlog命令来查看慢查询日志
 
 一般发生阻塞是因为慢操作, 像 大key操作
 或者是阻塞命令, 系统或网络上的阻塞, 也有可能是过期key集中失效,回收过期key阻塞

 阻塞排查先看 slowlog , 再看大key, 在排查客户端, 再看RDB/AOF的状态信息
 latenc doctor 延迟诊断工具
 
 ``redis
 redis-cli monitor  (当前命令是不是阻塞命令)
 redis-cli  slowlog get 10 
  redis-cli --bigkeys
  INfo clients
  INFO stats (redis异常状态)
  INFO persistence(AOF/RDB 状态信息)
  latency doctor
`` 
## 异步消息队列
 1. 使用List 和 LPUSH 和 RPOP 实现 
  因为是双向链表, 所以规定最近右出
  用Java代码实现异步消息队列, 用redisTemplete实现生产者和消费者的方法,和一个阻塞消费的方法(放开cpu)
2.  用 pub/sub 订阅机制实现广播: 发布消息到指定频道和订阅者订阅频道(redisTemplete实现) 
  但是这种方法就是吼一嗓子, 他做不到消息确认和保存消息,一旦redis出问题就会消息丢失
  同样有好处,消息是实时通知的,适合即时的场景
 
3. stream 实现消息队列, 支持消费者组(共享一条消息) ,有ACK, 存放消息实体
 
 ==以上方法都是redis MQ的实现方法,但是都有问题:==
  单线程的,吞吐量有限
  不适合大规模路由(路由决定的是消息的分发,谁可以接收到,)
  不能超大规模

## 延时消息队列
 消息不会被立刻消费,而是在一段时间后在被消费
 
 使用ZSet把延时时间 设置为score 字段, 在ZSet里就会根据延时时间给消息排序,按照时间顺序去进行消息处理

 ==常见的场景是 订单取消和定时提醒 ==
 
 技术派项目使用这种方式实现文章定时发布定时消息, 在期限到达后,消费这条消息然后发布文章 
 

## Redis 事务
 redis 支持简单的事务: multi(开启) ,exec(提交执行), discard(放弃), watch(乐观锁) 打包+ 顺序执行,
 
 不支持回滚,也没有隔离等级
 全靠 watch监控在exec之前的版本   检查key是否被修改


 Redis 的单线程保证了事务执行过程中不会有命令插队, ==保证执行顺序和不可被打断==, 但==不保证命令是否执行成功==

 redis事务的实现是用了一个事务队列, 在multi 之后的命令房间队列,先进先出, discard 将命令取出, exec 
开始顺序取出队列里的命令执行, 不会回滚

 ==事务并不会导致实际阻塞, 而是阻塞放大器, 执行一些大key操作或阻塞命令再加上大事务, 阻塞就会很明显了==
 == watch 重试会导致逻辑阻塞, 这是客户端的阻塞,不是redis的阻塞. 筛查起来看客户端的重试情况和 qps是否飙升==

### watch 
 watch 把key放到全局监视字典里, 如果发现Exec 之前key已经被写操作修改过, 就标记为dirty, 事务不会执行,返回nil
 
### 不支持回滚的原因: 
 

1. Redis 没有MVCC或undo log等基础设施, 而MySQL是依赖这些设施实现回滚的
2.   Redis 的设计目标是单线程,高性能, 引入回滚要保存状态信息, 并且要在发生错误时逆向执行命令恢复状态 , 每条命令都要记录之前的值,很大的内存和性能上的开销, 和设计目标相悖

3.  Redis的命令是有破坏性不可逆的(直接修改内存) ,这一点是为了迎合设计目标而采取的手段, 直接无法回滚

  ==这是架构上的取舍, 追求极致性能==

### Redis事务有部分原子性
 事务里 有一个命令执行失败不会导致事务回滚, 这也就不能满足 ==要么事务内容全部执行成功,要么全部失败并保持原来状态==
 
 Redis还有脚本命令, 使用lua脚本 能保证执行的原子性(合并成一个命令),但是中途失败被修改的也不能回滚

 ### Redis事务没有一致性
  一致性指的是从一个正确的状态 迁移到另一个正确的状态
 Redis 并不能满足, 库存实际上是不可能减成负数,但是redis没有这个约束 
 ==Redis 没有一致性约束==

### Redis 有一点隔离性
  Redis 没有隔离级别,没有MVCC , 没有锁机制, 仅依靠 Watch 乐观锁 

### Redis 有可调持久性
 开启RDB可能会 丢失几分钟
 开启AOF 会丢失秒内操作, 不过开了AOF always 很接近持久性

## pipeLine
 pipeline让客户端不用发一个等相应在发一个, 而是一次性发多个命令
 ==目的是减少网络往返== 
 
 大量命令和事务的执行缺点是一样的: 不能保证命令的成功执行

 而且pipeline太大会导致短暂阻塞, 也是阻塞的原因:
   一个请求发的popeline里面的命令太多,会导致短时间无法接收其他请求,QPS断崖下降

 ==一般只放1000到5000个命令==

##### pipeline 取消必须一次命令一次应答 , 允许客户端先将执行的命令写入缓冲, 最后一次性转发给redis客户端 ,这种方式让Redis提速, 也就是单次改批量
 通过减少响应次数来提高redis的性能 ,极端情况下可以提速70多倍
== pipeline 是客户端提供的,而不是服务器提供的==

 ## redis实现分布式锁
 使用setnx 命令 设置key value 以及 expire 有效期防止死锁
 redis是单线程的, 同一时间只有一个命令在执行, 两个客户端同时发请求, redis严格按照到达的顺序设置key(拿到锁)
 
### redis分布式锁的问题
 设置过期时间有可能在临界区代码没执行完线程A就释放了, 另一个线程B拿到了锁, 然后临界区代码删除key 删的是B 

 让锁自动续期解决锁过期
 Redission  的看门狗,后台启动定时任务,每过一段时间就检查锁是否被当前
线程持有,然后给线程延迟过期时间

## Redisson
 基于Redis的Java客户端, 提供很多分布式的数据结构和服务
 
### 看门狗机制
 当lock()没有显式指定过期时间,默认30秒过期. 自动开一个定时任务,每秒10w 检查锁是否是当前线程持有 (value 是 currentThreadId), 核对ID是否一致
 如果持有把过期时间重置为30秒

 ==当然,为了保证锁检查和续期操作的原子性,要用lua脚本确保==
 
 如果这个锁不被当前线程持有 ,看门狗就不会自动续期了,没有unlock 也不会有影响

 ## RedLock
 多个完全独立的 redis 实例上同时获取锁h
  如果 成功的实例数超过一般就认为是获取成功

 但是有时钟漂移问题(误差)和网络分区问题(访问不到)
 
 ==用概率对抗不确定的方案 ==

 但是有个很大的弊病: 当客户端发生GC 和STW 时, RedLock 感知不到客户端的状态
因为 RedLock 只能判断：

锁有没有过期 ,多数 Redis 是否同意

但它无法判断：

客户端是否“暂停过” , 客户端是否还“配得上”继续执行

Redis 是无状态的，它不知道：
 
#### 项目使用点:
  pmhub上确保流程状态的更新按顺序执行,不被其他服务打扰

# 底层结构 
 1. String -> SDS 
 2. hash -> ziplist/listpack/hashtable
 3. List -> qucikList
 4. Set -> intset(只有整数)/hashtable
 5. zset -> ziplist/ listpack + skiplist + dict 
  
### SDS 
 数据结构中有使用长度, 未使用长度,和字符数组
 ### dict 
 用俩哈希表, 第二个留给rehash用(动态扩容)
### listpack
 


### SkipList
 quickList 把链表分成多层, 最底层是完全链表, 上一层是i下面一层节点的一半, 查找过程类似二分查找, 时间复杂度是O(logn), 
链表越大越友好 ,==相当于浪费了链表上层存储空间,用快速二分查找节省了时间

#### Zset的字节小于128 元素值小于64的时候,使用ListPack进行支持,否则使SiipList + Dict
==dict 用来精确查询,SkipList用于范围查询==


# Reactor 网络模型
 分而治之 和 事件驱动 (之前的多路复用也提到过 epoll 也是事件驱动的 )

 通过将大任务分成若干个小任务的方式来减少阻塞时间, 达到提升性能的目的

一个连接的网络处理过程是: accept, read , decode, process, encode , send 

 Reactor 模式把每个步骤映射为一个task, 服务端线程执行的最小逻辑单元不是真个请求,而是TASK, 采用非阻塞式的方式执行

 事件驱动就是把相应的Task 对应特定的网络事件, TASK 准备就绪时,Reactor收到相应的网络事件通知,
把Task分发给绑定了对应网络事件的ACCeptor和 handler 执行
